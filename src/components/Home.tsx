...
export default function Home() {
  ...
  useEffect(() => {
    axios
      .get(`https://api.artic.edu/api/v1/artworks/129841`)
      .then((res) => {
        console.log(res);
  ...
  return (
    <div className="grow">
      <article className="bg-black text-white h-screen grid grid-cols-3 grid-rows-3">
        <div className="h-full border col-span-2 row-span-2 border-white">
          <img
            className="h-full w-full"
            src={`https://www.artic.edu/iiif/2/${art?.image_id}/full/843,/0/default.jpg`}
          />
        </div>
        <div className="h-full border row-span-2 border-white">
          <p>{art?.thumbnail.alt_text}</p>
        </div>
        <div className="h-full border col-span-2 border-white">
          <h1>{art?.title}</h1>
